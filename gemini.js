...
async function transcribeAudioWithGemini({ apiKey, audioBlob, contextHint }) {
  ...
  const instruction = [
    ...
    // v0.13.7: 独り言や小さい声は「拾うべき発話」なので必ず文字起こしする。
    // ただし、Gemini が音声を聞き取れなかった時に「指示プロンプトへの返事」として
    // 「はい、わかりました」を生成する事故は防ぎたい。
    // → 「音声内容に基づかない応答生成だけ」を禁止する書き方にする。
    '- 重要：あなたは音声の中身を文字に起こす立場で、このプロンプトに返事をする立場ではない。',
    '  音声の中身が聞き取れない・意味ある発話ゼロのときは、「はい」「わかりました」「了解しました」',
    '  といったプロンプト応答を生成せず、必ず空文字列のみ返すこと。',
    '  独り言や小さい声でも、実際の音声に含まれているなら文字起こしの対象とする（落としてはいけない）。',
  ].join('\n');
