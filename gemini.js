...
async function transcribeAudioWithGemini({ apiKey, audioBlob, contextHint }) {
  ...
  const instruction = [
    ...
    // ↓ v0.13.6: VB-Cable 経由など微弱音が混じる環境で、Gemini が指示プロンプトに対して',
    // 「はい、わかりました」と返事を生成する事故を防ぐためのガード。',
    '- 重要：音声の中身が聞き取れない、または十分な意味のある発話が無い場合は、',
    '  「はい」「わかりました」「もうダメだ」「やばい」などの独り言や応答を絶対に生成せず、',
    '  必ず空文字列のみを返してください。あなたはこのプロンプトに対して返事をする立場ではなく、',
    '  音声の中身を文字に起こす立場です。',
  ].join('\n');
