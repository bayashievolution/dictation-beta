 ...
 */
async function formatForOSDWithGemini({ apiKey, text, continuationMark = true }) {
  if (!apiKey) throw new Error('Gemini API キーが設定されていません');
  if (!text || !text.trim()) return '';

  // v0.13.31: continuationMark=true なら「→」継続マークを付ける、false なら付けない。
  // やっさん指示「AI字幕整形 ON 時のサブ設定でユーザーが選べるように」。
  const continuationLines = continuationMark
    ? ['- 文節の途中で改行する場合は、行末に「→」を付けて継続を示す（TV字幕と同じルール）']
    : ['- 文節の途中改行に「→」継続マークは付けない（OFF 設定）'];

  const instruction = [
    'あなたは聴覚障害のある方向けのTV字幕編集者です。',
    '以下の文字起こしテキスト（字幕バッファの最新数行）を、TV字幕風に整形してください。',
    '',
    '【最重要ルール】',
    '- **文字数はほぼ変えない**：読みやすさのために改行・句読点だけを調整する。',
    '  発話内容を要約・短縮・補足してはいけない。フィラー（えー、あのー）が文字起こしに含まれている場合も削除しない。',
    '- **あくまで文字起こし**なので、飛躍した推測や情報の追加は禁止。意味を変えない。',
    '',
    '【整形ルール】',
    '- 文節（意味のかたまり）の区切りで改行を入れて読みやすく',
    ...continuationLines,
    '- 句読点「、。」は維持。明らかな誤りだけ自然な位置に直す（過剰な追加は禁止）',
    '- 「## 見出し」などのMarkdown見出しは削除して本文だけ出力',
    '- 「（文字起こし中…）」「（音声不明瞭）」等のメタ表記は削除',
    '',
    '【文脈の活用】',
    '- 入力テキストの末尾に近い行ほど発話が新しい',
    '- ある程度文脈が積み重なっている場合は、話題の流れから自然な改行位置を判断する',
    '  （ただし話題の推測結果を本文に書き加えてはいけない、改行位置の判断材料にとどめる）',
    '',
    '【出力】',
